...
// Allows ordering by joined relation properties, e.g. `{ 'profile.bio': 'asc' }` when `profile` is joined
type QBOrderByHint<Hint extends string> = IsNever<Hint> extends true ? never : Record<`${Hint}.${string}`, QueryOrderKeysFlat>;
...
> {
  ...
  orderBy(orderBy: QBQueryOrderMap<Entity> | QBQueryOrderMap<Entity>[] | QBOrderByHint<Hint>): SelectQueryBuilder<Entity, RootAlias, Hint, Context> {
    return this.processOrderBy(orderBy, true);
  }

  andOrderBy(orderBy: QBQueryOrderMap<Entity> | QBQueryOrderMap<Entity>[] | QBOrderByHint<Hint>): SelectQueryBuilder<Entity, RootAlias, Hint, Context> {
    return this.processOrderBy(orderBy, false);
  }

  private processOrderBy(orderBy: QBQueryOrderMap<Entity> | QBQueryOrderMap<Entity>[] | QBOrderByHint<Hint>, reset = true): SelectQueryBuilder<Entity, RootAlias, Hint, Context> {
    this.ensureNotFinalized();
