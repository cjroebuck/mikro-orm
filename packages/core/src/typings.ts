 ...
 */
export type PolymorphicPrimary<T> = T extends object
  ? Primary<T> extends readonly [infer A, infer B]
    ? readonly [string, A, B] | [string, A, B]
...
export type RequiredEntityData<T, I = never, C extends boolean = false> = {
  [K in keyof T as RequiredKeys<T, K, I>]: T[K] | RequiredEntityDataProp<T[K], T, C> | Primary<T[K]> | PolymorphicPrimary<T[K]>
} & {
  [K in keyof T as OptionalKeys<T, K, I>]?: T[K] | RequiredEntityDataProp<T[K], T, C> | Primary<T[K]> | PolymorphicPrimary<T[K]> | null
};
