...
export class ObjectHydrator extends Hydrator {
  ...
  getEntityHydrator<T extends object>(meta: EntityMetadata<T>, type: 'full' | 'reference', normalizeAccessors = false): EntityHydrator<T> {
    ...
    const hydrateToOne = (prop: EntityProperty, dataKey: string, entityKey: string) => {
      ...
      // For polymorphic: PolymorphicRef.is() check; for regular: isPrimaryKey() check
      const pkCheck = prop.polymorphic ? `PolymorphicRef.is(data${dataKey})` : `isPrimaryKey(data${dataKey}, true)`;
      ret.push(`    if (${pkCheck}) {`);
      ...
      if (prop.polymorphic) {
        ...
        ret.push(`      if (!targetClass) throw new Error(\`Unknown discriminator value '\${data${dataKey}.discriminator}' for polymorphic relation '${prop.name}'. Valid values: \${Object.keys(${discriminatorMapKey}).join(', ')}\`);`);
        if (prop.ref) {
